...
def update_schedule_log_on_visit(doc, method):
    # Loop through items in the maintenance visit
    for item in doc.purposes:
        # Try to find the corresponding Schedule Log
        log = frappe.get_all("Schedule Log",
            filters={
                "maintenance_schedule": doc.maintenance_schedule,
				"device":item.device,
        ...
        if log:
            log_doc = frappe.get_doc("Schedule Log", log[0].name)
            log_doc.actual_date = doc.mntc_date  # or any relevant date field
            log_doc.completion_status = doc.completion_status  # You might get this from `doc.status` or set manually
            log_doc.save()
            frappe.msgprint(f"Updated Schedule Log: {log_doc.name}")


def update_teammember_tasks_on_visit(doc, method):
    for item in doc.purposes:
        if not item.teammember_id:
            frappe.msgprint(f"⚠️ No Team Member assigned for item: {item.item_code}")
            continue

        frappe.msgprint(f"🔍 Checking Team Member: {item.teammember_id}")

        try:
            emp_doc = frappe.get_doc("Team Member", item.teammember_id)
        except frappe.DoesNotExistError:
            frappe.msgprint(f"❌ Team Member {item.teammember_id} not found!")
            continue

        frappe.msgprint(f"✅ Found Team Member: {emp_doc.name}")

        task_updated = False
        for task in emp_doc.get("tasks"):
            # Debug: Print task details
            frappe.msgprint(
                f"Task Details:\n"
                f"- Maintenance Schedule: {task.maintenance_schedule}\n"
                f"- Item Code: {task.item_code}\n"
                f"- Scheduled Date: {task.scheduled_date}\n"
                f"Expected:\n"
                f"- Schedule: {doc.maintenance_schedule}\n"
                f"- Item: {item.item_code}\n"
                f"- Date: {item.scheduled_date}"
            )

            if (
                task.maintenance_schedule == doc.maintenance_schedule
                and task.item_code == item.item_code
                and str(task.scheduled_date) == str(item.scheduled_date)
            ):
                task.actual_date = doc.mntc_date or doc.completion_date or doc.posting_date
                task.completion_status = doc.completion_status or "Completed"
                emp_doc.save()
                frappe.msgprint(f"🔄 Updated task for {emp_doc.name}")
                task_updated = True
                break

        if not task_updated:
            frappe.msgprint("❌ No matching task found!")