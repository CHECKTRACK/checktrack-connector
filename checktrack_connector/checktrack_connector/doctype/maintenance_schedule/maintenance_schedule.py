	...
	def generate_schedule(self):
		...
		for d in self.get("items"):
			...
			for i in range(d.no_of_visits):
				...
				child.teammember_id = d.teammember_id
				child.teammember_name = d.teammember_name
...
def add_amc_expiry_date_in_device(doc, method):
    for item in doc.items:
        if item.device:
            # Find Device by serial_no
            devices = frappe.get_all("Device", filters={"serial_no": item.device}, fields=["name"])
            if not devices:
            ...
            device.save()
            frappe.msgprint(f"Updated AMC expiry for {device.serial_no}")
...
def create_schedule_logs(doc, method):
		for row in doc.schedules:
			...
			log.assign_to_id = row.teammember_id
			log.assign_to = row.teammember_name
			...
			if row.teammember_id:
				teammember = row.teammember_id.strip()
				teammember = frappe.get_list(
					"Team Member",
					filters={"teammember_id": teammember},
					fields=["name"],
					limit=1
				)

				if teammember:
					emp_doc = frappe.get_doc("Team Member", teammember[0].name)

					task_row = emp_doc.append("tasks", {})
					task_row.device = row.device
					...
					emp_doc.save()
					frappe.msgprint(f"Task added for Team Member (id): {teammember}")
				else:
					frappe.log_error(f"Team Member with id '{teammember}' not found.", "Task Creation Failed")