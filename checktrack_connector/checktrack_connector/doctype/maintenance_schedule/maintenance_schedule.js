...
frappe.ui.form.on("Maintenance Schedule", {
	...
	customer: function (frm) {
		...
		if (frm.doc.customer) {
			// Update 'items' table (replace with your child table name)
			if (frm.doc.items && frm.doc.items.length > 0) {
				frm.doc.items.forEach(item => {
					frappe.model.set_value(item.doctype, item.name, "customer_name", frm.doc.customer);
				});
...
checktrack_connector.maintenance.MaintenanceSchedule = class MaintenanceSchedule extends frappe.ui.form.Controller {
	refresh() {
...
extend_cscript(cur_frm.cscript, new checktrack_connector.maintenance.MaintenanceSchedule({ frm: cur_frm }));
